...
- To wire gsd-workflow MCP (and optionally playbook-gsd), use `scripts/bootstrap-gsd-workflow.sh` with `--patch-mcp` (and usually `--init-gsd`); `--with-do-next` also copies `shared/gsd/scripts/playbook-gsd-health.sh` into client `.workflow/scripts/`. stderr warnings for `GSD_WORKFLOW_EXECUTORS_MODULE` / `GSD_WORKFLOW_WRITE_GATE_MODULE` are expected for local `~/.gsd` bridges. Client delivery settings live in `.gsd/DELIVERY-PROFILE.md`; re-bootstrap without `--force` will not overwrite an existing profile.
